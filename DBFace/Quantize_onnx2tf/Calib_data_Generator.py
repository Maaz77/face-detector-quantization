...
def display_random_images(data: np.ndarray = None, file_path: str = None, percentage: float = 10, is_nchw: bool = False) -> None:
    ...
    for i, idx in enumerate(indices):
        ...
        img = data[idx].copy()  # Create a copy to avoid modifying original data
        
        # Convert float to uint8 if needed for cv2.cvtColor
        ...
        ax.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))  # Convert back to RGB for matplotlib
...
def gen_calibdata_from_webcamv2(
    output_file_path: str = 'webcam_calibdata.npy',
    sample_size: int = 256,
    capture_frequency_ms: float = 200,
    camera_id: int = 0,
    ...
    """
    Generate and save calibration data from webcam captures.
    
    Args:
        output_file_path: Path to save the calibration data
        sample_size: Number of frames to capture (default: 256)
        capture_frequency_ms: Time between captures in milliseconds
        camera_id: Camera ID to use (default: 0 for default webcam)
        display_percentage: Percentage of images to display after capture
        normalize: Whether to normalize the data (default: False)
        output_format: Format of output data - 'NCHW' or 'NHWC' (default: 'NHWC')
        resolution: Tuple of (height, width) for image resolution (default: (480, 640))
    """
    import time
    import os
    
    if output_format not in ['NCHW', 'NHWC']:
        raise ValueError("output_format must be either 'NCHW' or 'NHWC'")
    ...
    print(f"Capturing {sample_size} frames at {capture_frequency_ms}ms intervals...")
    print(f"Resolution: {height}x{width}, Format: {output_format}")
    ...
    try:
        while frames_captured < sample_size:
            ...
            # Resize to ensure correct resolution
    ...
    # Stack all processed frames (currently in NHWC format)
    calib_datas = np.stack(processed_frames)
    
    # Print raw data statistics
    print("\nRaw data statistics:")
    print_stats(calib_datas)
    print(f"Min value: {np.min(calib_datas)}, Max value: {np.max(calib_datas)}")
    
    if normalize:
        # Apply normalization using the existing function
        calib_datas, mean_per_channel, std_per_channel = normalize_data(calib_datas)
        
        print("\nStatistics after normalization:")
        print_stats(calib_datas)
        print(f"Min value: {np.min(calib_datas)}, Max value: {np.max(calib_datas)}")
    else:
        # No normalization, just scale by dividing by 255.0
        calib_datas = calib_datas / 255.0
        
        print("\nStatistics after scaling to [0,1]:")
        print_stats(calib_datas)
        print(f"Min value: {np.min(calib_datas)}, Max value: {np.max(calib_datas)}")
    ...
    print(f'Final calibration data shape: {calib_datas.shape}')
    ...
    if display_percentage > 0:
        display_random_images(
            data=calib_datas, 
            percentage=display_percentage,
            is_nchw=(output_format == 'NCHW')
        )
import cv2

def release_all_cameras(max_camera_id=10):
    """
    Attempt to release all potentially open cameras.
    
    Args:
        max_camera_id: Maximum camera ID to attempt to release
    """
    print("Attempting to release all cameras...")
    
    for camera_id in range(0):
        try:
            # Try to open and immediately release the camera
            cap = cv2.VideoCapture(camera_id)
            cap.release()
            print(f"Successfully released camera {camera_id}")
        except:
            print(f"No camera found at ID {camera_id} or could not release it")
    
    print("Camera release process complete")


if __name__ == "__main__":
    
    gen_calibdata_from_webcamv2(
        output_file_path='Quantize_onnx2tf/calibdata_NHWC_480_640_DBFace.npy',
        sample_size=256,
        capture_frequency_ms=300,
        camera_id=0,
